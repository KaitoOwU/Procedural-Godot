...
func _spawnEntitiesInZone() -> void:
	...
	for t in enemiesNumber:
		if waterTiles.size() == 0:
			return
		var enemy = QuestGenerator.enemy_list.pick_random()
		var enemyBase = enemy.sceneRef
		var obj : Enemy = enemyBase.instantiate()
		add_child(obj)
		obj.enemy_type = enemy
	...
	for t in collectiblesNumber:
		if sandTiles.size() == 0:
			return
		var collectible = QuestGenerator.collectible_list.pick_random()
		var collectibleBase = collectible.sceneRef			
		var obj : CollectibleBase = collectibleBase.instantiate()
		add_child(obj)
		obj.collectible_type = collectible
